...
void change_cache(int address_physical){
	...
	int setindex  = (address_physical & 0x00000FF0) >> 4; // offset setindex 
...
void printCache(){
		...
		for(unsigned int i = 0; i < PhysicalCache.size(); ++i){
		  if(PhysicalCache[i].cache_tag != -1){
			...
			cout << "[0x" << hex << setw(8) << setfill('0') << (PhysicalCache[i].cache_tag<<12)+(i<<4)+0x0 <<  "-0x" << hex << setw(8) << setfill('0') << (PhysicalCache[i].cache_tag<<12)+(i<<4)+0xF << "]";
