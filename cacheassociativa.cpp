...
vector<memory_cache> CacheWay0;
vector<memory_cache> CacheWay1;
vector<memory_cache> CacheWay2;
vector<memory_cache> CacheWay3;
...
void change_cache(int address_physical){
	...
	int setindex  = (address_physical & 0x000003F0) >> 4; // offset setindex

	
		if(CacheWay0[setindex].cache_tag == cache_tag){
			CacheWay0[setindex].hit++;
		}else if(CacheWay1[setindex].cache_tag == cache_tag){
			CacheWay0[setindex].miss++;
			CacheWay1[setindex].hit++;
		}else if(CacheWay2[setindex].cache_tag == cache_tag){
			CacheWay0[setindex].miss++;
			CacheWay1[setindex].miss++;
			CacheWay2[setindex].hit++;
		}else if(CacheWay3[setindex].cache_tag == cache_tag){
			CacheWay0[setindex].miss++;
			CacheWay1[setindex].miss++;
			CacheWay2[setindex].miss++;
			CacheWay3[setindex].hit++;
		}else{
			CacheWay0[setindex].miss++;
			CacheWay1[setindex].miss++;
			CacheWay2[setindex].miss++;
			CacheWay3[setindex].miss++;
			if(cot % 4 == 0){
				CacheWay0[setindex].cache_tag = cache_tag;
			}else if(cot % 4 == 1){
				CacheWay1[setindex].cache_tag = cache_tag;
			}else if(cot % 4 == 2){
				CacheWay2[setindex].cache_tag = cache_tag;
			}else if(cot % 4 == 3){
				CacheWay3[setindex].cache_tag = cache_tag;
			}					
			cot++;
		}	
...
void printCache(){
		cout << "WAY 0" << endl;	
		for(unsigned int i = 0; i < CacheWay0.size(); ++i){
		  if(CacheWay0[i].cache_tag != -1){
			cout << hex << i << ": ";				
			cout << "[0x" << uppercase << hex << setw(8) << setfill('0') << CacheWay0[i].cache_tag << "]";
			cout << "[0x" << hex << setw(8) << setfill('0') << (CacheWay0[i].cache_tag<<10)+(i<<4)+0x0 <<  "-0x" << hex << setw(8) << setfill('0') << (CacheWay0[i].cache_tag<<10)+(i<<4)+0xF << "]";
			cout << " H(" << dec << CacheWay0[i].hit << ")";
			cout << " M(" << CacheWay0[i].miss << ")" << endl;
		  
		  }	
		}
		printf("\n\n");
		cout << "WAY 1" << endl;
		for(unsigned int i = 0; i < CacheWay1.size(); ++i){
		  if(CacheWay1[i].cache_tag != -1){
			cout << hex << i << ": ";				
			cout << "[0x" << uppercase << hex << setw(8) << setfill('0') << CacheWay1[i].cache_tag << "]";
			cout << "[0x" << hex << setw(8) << setfill('0') << (CacheWay1[i].cache_tag<<10)+(i<<4)+0x0 <<  "-0x" << hex << setw(8) << setfill('0') << (CacheWay1[i].cache_tag<<10)+(i<<4)+0xF << "]";
			cout << " H(" << dec << CacheWay1[i].hit << ")";
			cout << " M(" << CacheWay1[i].miss << ")" << endl;
		...
		printf("\n\n");
		cout << "WAY 2" << endl;
		for(unsigned int i = 0; i < CacheWay2.size(); ++i){
		  if(CacheWay2[i].cache_tag != -1){
			cout << hex << i << ": ";				
			cout << "[0x" << uppercase << hex << setw(8) << setfill('0') << CacheWay2[i].cache_tag << "]";
			cout << "[0x" << hex << setw(8) << setfill('0') << (CacheWay2[i].cache_tag<<10)+(i<<4)+0x0 <<  "-0x" << hex << setw(8) << setfill('0') << (CacheWay2[i].cache_tag<<10)+(i<<4)+0xF << "]";
			cout << " H(" << dec << CacheWay2[i].hit << ")";
			cout << " M(" << CacheWay2[i].miss << ")" << endl;
		...
		printf("\n\n");
		cout << "WAY 3" << endl;
		for(unsigned int i = 0; i < CacheWay3.size(); ++i){
		  if(CacheWay3[i].cache_tag != -1){
			cout << hex << i << ": ";				
			cout << "[0x" << uppercase << hex << setw(8) << setfill('0') << CacheWay3[i].cache_tag << "]";
			cout << "[0x" << hex << setw(8) << setfill('0') << (CacheWay3[i].cache_tag<<10)+(i<<4)+0x0 <<  "-0x" << hex << setw(8) << setfill('0') << (CacheWay3[i].cache_tag<<10)+(i<<4)+0xF << "]";
			cout << " H(" << dec << CacheWay3[i].hit << ")";
			cout << " M(" << CacheWay3[i].miss << ")" << endl;
...
int main() {
	//initializing the memory cache;
		CacheWay0.resize(64,memory_cache());
		CacheWay1.resize(64,memory_cache());
		CacheWay2.resize(64,memory_cache());
		CacheWay3.resize(64,memory_cache());
	//end
