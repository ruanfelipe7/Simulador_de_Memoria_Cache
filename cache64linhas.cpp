...
void change_cache(int address_physical){
	int cache_tag = (address_physical & 0xFFFFF000) >> 12; // offset tag
	int setindex  = (address_physical & 0x00000FC0) >> 6; // offset setindex 
...
void printCache(){
		...
		for(unsigned int i = 0; i < PhysicalCache.size(); ++i){
		  if(PhysicalCache[i].cache_tag != -1){
			...
			cout << "[0x" << hex << setw(8) << setfill('0') << (PhysicalCache[i].cache_tag<<12)+(i<<6)+0x0 <<  "-0x" << hex << setw(8) << setfill('0') << (PhysicalCache[i].cache_tag<<12)+(i<<6)+0x3F << "]";
...
int main() {
	//initializing the memory cache;
		PhysicalCache.resize(64,memory_cache());
	//end
